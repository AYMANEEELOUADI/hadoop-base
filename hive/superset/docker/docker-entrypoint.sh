...
if [[ -n "$MYSQL_PORT" && -n "$MYSQL_HOST" && -n "$MYSQL_DIALECT" && -n "$MYSQL_DB" && -n "$MYSQL_USER" && -n "$MYSQL_PASSWORD" ]]

then
  echo Exporting MYSQL_URL environment variable
  export SQLALCHEMY_DATABASE_URI="$MYSQL_DIALECT://$MYSQL_USER:$MYSQL_PASSWORD@$MYSQL_HOST:$MYSQL_PORT/$MYSQL_DB"
